...
const getSortableDate = (dStr) => {
  if (!dStr) return 0;
  if (String(dStr).includes('-')) return new Date(dStr).getTime();
...
export default function App() {
  ...
  const checkAndAddCustomer = (customerName) => {
    ...
    const exists = customers.some(
      (c) =>
        ...
        String(customerName).toLowerCase().trim()
    );
    if (!exists && String(customerName).trim() !== '') {
      ...
      logAction(
        'Auto-Add',
        'Master Customer',
        `Added ${String(customerName).trim()}`
      );
    }
  };
          ...
          <form
            onSubmit={(e) => {
              ...
              const user = usersList.find(
                (u) => u.username === un && u.password === pw
              );
              if (user) {
                if (!user.active)
                  return alert('Account deactivated. Contact Admin.');
                setCurrentUser(user);
                logAction(
                  'Login',
                  'System',
  ...
  const Dashboard = () => {
    ...
    const urgentAlerts = activeCheques.filter(
      (c) => c.status === 'Bounced' || c.mismatchAlert
    );
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          <div className="bg-white p-5 rounded shadow border-l-4 border-green-500 hover:shadow-lg transition-shadow">
            ...
            <p className="text-2xl font-mono font-bold text-gray-800">
              {formatCurrency(
                cleared.reduce((sum, c) => sum + (c.amount || 0), 0)
              )}
            </p>
            <p className="text-sm text-green-600 mt-1">
              {cleared.length} system cheques
            </p>
          </div>
          <div className="bg-white p-5 rounded shadow border-l-4 border-yellow-500 hover:shadow-lg transition-shadow">
            ...
            <p className="text-2xl font-mono font-bold text-gray-800">
              {formatCurrency(
                pending.reduce((sum, c) => sum + (c.amount || 0), 0)
              )}
            </p>
            <p className="text-sm text-yellow-600 mt-1">
              {pending.length} system cheques
            </p>
          </div>
          <div className="bg-white p-5 rounded shadow border-l-4 border-red-500 hover:shadow-lg transition-shadow">
            ...
            <p className="text-2xl font-mono font-bold text-red-600">
              {formatCurrency(
                bounced.reduce((sum, c) => sum + (c.amount || 0), 0)
              )}
            </p>
            <p className="text-sm text-red-600 mt-1">
              {urgentAlerts.length} Action Required
            </p>
          </div>
        ...
        {urgentAlerts.length === 0 ? (
          <div className="bg-green-50 rounded shadow p-6 text-center border border-green-200">
            ...
            <p className="text-green-800 font-bold">
              All clear! No urgent mismatches currently.
            </p>
          </div>
        ) : (
          <div className="bg-white rounded shadow p-4 border border-red-200">
            {urgentAlerts.map((c) => (
              <div
                key={c.id}
                className="flex justify-between items-center p-4 bg-red-50 rounded mb-2 border border-red-100"
              >
                <div>
                  ...
                  <p className="text-sm text-red-600 font-medium mt-1">
                    System Amount: {formatCurrency(c.amount)} |{' '}
                    {c.mismatchAlert || 'Cheque Bounced'}
                  </p>
                </div>
                <button
      ...
      reader.onload = (evt) => {
        try {
          ...
          if (type === 'ICICI') {
            ...
            if (headerRowIdx === -1)
              return alert(
                'Error: Could not find ICICI column headers in this file.'
              );

            const header = rawData[headerRowIdx];
            const cId = header.findIndex((h) => h.includes('Transaction ID'));
            const cDate = header.findIndex(
              (h) => h.includes('Value Date') || h.includes('Txn Posted Date')
            );
            ...
            alert(
              `Parsed ICICI Statement successfully!\n\nNew records added: ${newAdded}\nDuplicates skipped: ${dupesSkipped}`
            );
            logAction(
              'Upload',
              'Bank Statement',
              `Imported ICICI file: ${file.name}`
            );
          } else if (type === 'TALLY') {
            const headerRowIdx = rawData.findIndex(
              (r) =>
                r.includes('Particulars') &&
                (r.includes('Vch No.') || r.includes('Vch Type'))
            );
            if (headerRowIdx === -1)
              return alert(
                'Error: Could not find Tally column headers in this file.'
              );
            ...
            for (let i = headerRowIdx + 1; i < rawData.length; i++) {
              ...
              if (
                part1.toLowerCase() === 'cr' ||
                part1.toLowerCase() === 'dr' ||
                part1 === ''
              ) {
              ...
              if (
                amt > 0 &&
                actualName !== 'Opening Balance' &&
                !actualName.toLowerCase().includes('suspense') &&
                vType.toLowerCase().includes('receipt')
              ) {
            ...
            alert(
              `Parsed Tally Export successfully!\n\nNew vouchers added: ${newAdded}\nDuplicates skipped: ${dupesSkipped}`
            );
            logAction(
              'Upload',
              'Tally Data',
              `Imported Tally file: ${file.name}`
            );
          }
        } catch (err) {
          alert(
            "Error parsing Excel file. Ensure it's not protected or corrupted."
          );
        }
      };
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="bg-white shadow rounded p-6 border-t-4 border-orange-500 relative">
            {isAdmin && bankData.length > 0 && (
              <button
                onClick={() => {
                  if (
                    window.confirm(
                      'WARNING: This will wipe all Bank Statement records. Continue?'
                    )
                  ) {
                    updateBankData([]);
            ...
            <h3 className="font-bold text-lg mb-2 flex items-center">
              <UploadCloud className="mr-2 text-orange-500" /> ICICI Statement
              (.xls)
            </h3>
            <p className="text-xs text-gray-500 mb-4">
              Auto-detects headers. Skips duplicate Transaction IDs.
            </p>
            <label className="border-2 border-dashed border-gray-300 rounded-lg p-8 flex flex-col items-center cursor-pointer hover:bg-orange-50 transition-colors">
              ...
              <input
                ...
                accept=".xls,.xlsx"
                className="hidden"
                onChange={(e) => processUpload(e, 'ICICI')}
              />
            </label>
            {bankData.length > 0 && (
              <p className="mt-4 text-green-600 font-bold text-sm">
                ✓ {bankData.length} deposits currently in memory.
              </p>
            )}
          </div>
          ...
          <div className="bg-white shadow rounded p-6 border-t-4 border-blue-500 relative">
            {isAdmin && tallyData.length > 0 && (
              <button
                onClick={() => {
                  if (
                    window.confirm(
                      'WARNING: This will wipe all Tally records. Continue?'
                    )
                  ) {
                    updateTallyData([]);
            ...
            <h3 className="font-bold text-lg mb-2 flex items-center">
              <FileSpreadsheet className="mr-2 text-blue-500" /> Tally Export
              (.xlsx)
            </h3>
            <p className="text-xs text-gray-500 mb-4">
              Smart Parser extracts real Party Names. Skips duplicate Vouchers.
            </p>
            <label className="border-2 border-dashed border-gray-300 rounded-lg p-8 flex flex-col items-center cursor-pointer hover:bg-blue-50 transition-colors">
              ...
              <input
                ...
                accept=".xls,.xlsx"
                className="hidden"
                onChange={(e) => processUpload(e, 'TALLY')}
              />
            </label>
            {tallyData.length > 0 && (
              <p className="mt-4 text-green-600 font-bold text-sm">
                ✓ {tallyData.length} vouchers currently in memory.
              </p>
            )}
          </div>
        </div>
  ...
  const Reconciliation = () => {
    ...
    cheques
      .filter((c) => !c.deleted && !matchedSysIds.has(c.id))
      .forEach((sysChq) => {
        ...
        const bMatch = bankData.find(
          (b) =>
            ...
            ((b.chqNo && safeChqNo && String(b.chqNo).includes(safeChqNo)) ||
              ...
              (b.amount === sysChq.amount &&
                String(b.desc).toLowerCase().includes(customerFirstWord)))
        );

        const tMatch = tallyData.find(
          (t) =>
            !matchedTallyIds.has(t.id) &&
            t.amount === sysChq.amount &&
    ...
    bankData.forEach((b) => {
      if (!matchedBankIds.has(b.id)) {
        const tMatch = tallyData.find(
          (t) => !matchedTallyIds.has(t.id) && t.amount === b.amount
        );
        if (tMatch) {
    ...
    if (reconFilter === 'Mapped')
      filteredRows = reconRows.filter(
        (r) =>
          !r.reconStatus.includes('Unmapped') && r.reconStatus !== 'System Only'
      );
    else if (reconFilter === 'Unmapped')
      filteredRows = reconRows.filter(
        (r) =>
          r.reconStatus.includes('Unmapped') || r.reconStatus === 'System Only'
      );
    else if (reconFilter !== 'All')
      filteredRows = reconRows.filter((r) => r.reconStatus === reconFilter);

    if (searchTerm) {
      const q = String(searchTerm).toLowerCase();
      filteredRows = filteredRows.filter(
        (r) =>
          String(r.sysChq?.customer || '')
            .toLowerCase()
            .includes(q) ||
          String(r.bMatch?.desc || '')
            .toLowerCase()
            .includes(q) ||
          String(r.tMatch?.particulars || '')
            .toLowerCase()
            .includes(q) ||
          String(r.sysChq?.chqNo || '')
            .toLowerCase()
            .includes(q) ||
          String(r.bMatch?.chqNo || '')
            .toLowerCase()
            .includes(q) ||
          String(r.tMatch?.vchNo || '')
            .toLowerCase()
            .includes(q)
      );
    }
    ...
    filteredRows.sort((a, b) => {
      ...
      if (sortConfig.key === 'date') {
        valA = getSortableDate(
          a.sysChq?.chqDate || a.bMatch?.txnDate || a.tMatch?.date
        );
        valB = getSortableDate(
          b.sysChq?.chqDate || b.bMatch?.txnDate || b.tMatch?.date
        );
      } else if (sortConfig.key === 'amount') {
        valA = a.sysChq?.amount || a.bMatch?.amount || a.tMatch?.amount || 0;
        valB = b.sysChq?.amount || b.bMatch?.amount || b.tMatch?.amount || 0;
      }
    ...
    const handleSaveMapping = (e) => {
      ...
      if (mapModal.action === 'CREATE') {
        ...
        if (!newMap.sysId && !newMap.bankId && !newMap.tallyId)
          return alert('Select at least one item to map to.');
        ...
        if (mapModal.row.manual) {
          const updatedMappings = manualMappings.filter(
            (m) => m.id !== mapModal.row.mappingId
          );
          updateMappings([...updatedMappings, newMap]);
          logAction(
            'Mapping',
            'Break Manual',
            `Broke manual match. Reason: ${reason}`
          );
          setMapModal(null);
          return;
        }
      ...
      logAction(
        'Mapping',
        mapModal.action,
        `Modified mapping. Reason: ${reason}`
      );
      setMapModal(null);
    };
    ...
    const handleExport = () => {
      ...
      const exportData = filteredRows.map((r) => ({
        ...
        'Sys Date': r.sysChq ? r.sysChq.chqDate : '-',
    ...
    return (
      <div className="p-6 flex flex-col h-full">
        <div className="flex justify-between items-center mb-4 shrink-0">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Excel-style view. Auto-linking System Cheques, Bank Statements,
              and Tally Exports.
            </p>
          </div>
          <div className="flex space-x-3">
            <button
              ...
              className="bg-green-600 text-white px-4 py-2 rounded text-sm font-bold flex items-center shadow hover:bg-green-700"
            >
            ...
            <button
              ...
              className="bg-blue-600 text-white px-4 py-2 rounded text-sm font-bold flex items-center shadow hover:bg-blue-700"
            >
              <RefreshCw
                size={16}
        ...
        <div className="grid grid-cols-3 gap-4 mb-4 shrink-0">
          <div
            onClick={() => setReconFilter('Mapped')}
            className={`border p-4 rounded text-center cursor-pointer transition-all shadow-sm ${
              reconFilter === 'Mapped'
            ...
            <p className="text-2xl font-bold text-green-600">
              {
                reconRows.filter(
                  (r) =>
                    !r.reconStatus.includes('Unmapped') &&
                    r.reconStatus !== 'System Only'
                ).length
              }
            </p>
          </div>
          <div
            onClick={() => setReconFilter('Unmapped Bank')}
            className={`border p-4 rounded text-center cursor-pointer transition-all shadow-sm ${
              reconFilter === 'Unmapped Bank'
          ...
          >
            ...
            <p className="text-2xl font-bold text-orange-600">
              {
                reconRows.filter((r) => r.reconStatus === 'Unmapped Bank')
                  .length
              }
            </p>
          </div>
          <div
            onClick={() => setReconFilter('Unmapped Tally')}
            className={`border p-4 rounded text-center cursor-pointer transition-all shadow-sm ${
              reconFilter === 'Unmapped Tally'
          ...
          >
            ...
            <p className="text-2xl font-bold text-purple-600">
              {
                reconRows.filter((r) => r.reconStatus === 'Unmapped Tally')
                  .length
              }
            </p>
          </div>
        ...
        <div className="flex justify-between items-center mb-4 shrink-0">
          <div className="flex space-x-3 w-1/2">
            <div className="relative w-full">
              ...
              <input
                type="text"
                placeholder="Search Names, Amounts, Refs..."
                className="border p-2 pl-9 rounded text-sm w-full shadow-sm focus:ring-2 focus:ring-blue-500"
                value={searchTerm}
          ...
          <select
            className="border p-2 rounded text-sm bg-white font-bold shadow-sm"
            value={reconFilter}
          ...
          <div className="overflow-auto flex-1">
            <table className="w-full text-left text-sm min-w-[1600px]">
              <thead className="bg-slate-800 text-white sticky top-0 z-10">
                <tr>
                  <th className="p-3 w-40 border-r border-slate-700">
                    Match Status
                  </th>
                  <th
                    className="p-3 border-r border-slate-700 bg-slate-700 cursor-pointer hover:bg-slate-600"
                    onClick={() => handleSort('date')}
                  >
                    Sys Date{' '}
                    <ArrowUpDown size={12} className="inline opacity-50" />
                  </th>
                  <th className="p-3 border-r border-slate-700 bg-slate-700">
                    Sys Customer
                  </th>
                  <th className="p-3 border-r border-slate-700 bg-slate-700">
                    Sys Chq
                  ...
                  <th
                    className="p-3 border-r border-slate-900 bg-slate-700 text-right cursor-pointer hover:bg-slate-600"
                  ...
                  <th
                    className="p-3 border-r border-slate-700 bg-slate-600 cursor-pointer hover:bg-slate-500"
                    onClick={() => handleSort('date')}
                  >
                    Bank Date{' '}
                    <ArrowUpDown size={12} className="inline opacity-50" />
                  </th>
                  <th className="p-3 border-r border-slate-700 bg-slate-600">
                    Bank Desc
                  </th>
                  <th className="p-3 border-r border-slate-700 bg-slate-600">
                    Bank Ref
                  ...
                  <th
                    className="p-3 border-r border-slate-900 bg-slate-600 text-right cursor-pointer hover:bg-slate-500"
                    onClick={() => handleSort('amount')}
                  >
                    Bank Amt{' '}
                    <ArrowUpDown size={12} className="inline opacity-50" />
                  ...
                  <th
                    className="p-3 border-r border-slate-700 bg-slate-500 cursor-pointer hover:bg-slate-400"
                    onClick={() => handleSort('date')}
                  >
                    Tally Date{' '}
                    <ArrowUpDown size={12} className="inline opacity-50" />
                  </th>
                  <th className="p-3 border-r border-slate-700 bg-slate-500">
                    Tally Particulars
                  </th>
                  <th className="p-3 border-r border-slate-700 bg-slate-500">
                    Tally Vch
                  ...
                  <th
                    className="p-3 border-r border-slate-900 bg-slate-500 text-right cursor-pointer hover:bg-slate-400"
                    onClick={() => handleSort('amount')}
                  >
                    Tally Amt{' '}
                    <ArrowUpDown size={12} className="inline opacity-50" />
              ...
              <tbody>
                {filteredRows.slice(0, 150).map((r) => (
                  <tr
                    key={r.id}
                    className={`border-b ${r.color} hover:opacity-80`}
                  >
                    <td className="p-3 border-r">
                      <span
                        className={`px-2 py-1 rounded text-xs font-bold ${r.statusBadge} block w-max`}
                      >
                        {r.reconStatus}
                      </span>
                    </td>
                    {r.sysChq ? (
                      <>
                        <td className="p-3 border-r whitespace-nowrap align-top">
                          {r.sysChq.chqDate}
                        </td>
                        <td className="p-3 border-r font-bold align-top">
                          {r.sysChq.customer}
                        </td>
                        <td className="p-3 border-r font-mono text-xs align-top">
                          {r.sysChq.chqNo}
                        </td>
                        <td className="p-3 border-r font-mono font-bold text-right align-top">
                          {formatCurrency(r.sysChq.amount)}
                        </td>
                      </>
                    ) : (
                      <td
                        colSpan="4"
                        className="p-3 border-r text-center align-middle bg-[url('data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSI4IiBoZWlnaHQ9IjgiPgo8cmVjdCB3aWR0aD0iOCIgaGVpZ2h0PSI4IiBmaWxsPSIjZmZmZmZmIj48L3JlY3Q+CjxwYXRoIGQ9Ik0wIDBMOCA4Wk04IDBMMCA4WiIgc3Ryb2tlPSIjZjNmNGY2IiBzdHJva2Utd2lkdGg9IjEiPjwvcGF0aD4KPC9zdmc+')]"
                      >
                        <span className="bg-white px-2 py-1 text-gray-400 text-xs font-bold uppercase rounded opacity-60">
                          No Sys Cheque
                        </span>
                      </td>
                    )}
                    <td className="p-3 border-r align-top bg-white bg-opacity-40 whitespace-nowrap">
                      {r.bMatch?.txnDate || '-'}
                    </td>
                    <td
                      className="p-3 border-r text-xs leading-tight align-top bg-white bg-opacity-40"
                      title={r.bMatch?.desc}
                    >
                      {r.bMatch?.desc || '-'}
                    </td>
                    <td className="p-3 border-r font-mono text-xs align-top bg-white bg-opacity-40">
                      {r.bMatch?.chqNo || '-'}
                    </td>
                    <td
                      className={`p-3 border-r font-mono font-bold text-blue-800 text-right align-top bg-white bg-opacity-40 ${
                        r.sysChq &&
                        r.bMatch &&
                        r.sysChq.amount !== r.bMatch.amount
                          ? 'text-red-600'
                          : ''
                      }`}
                    >
                      {r.bMatch ? formatCurrency(r.bMatch.amount) : '-'}
                    </td>
                    <td className="p-3 border-r align-top bg-white bg-opacity-20 whitespace-nowrap">
                      {r.tMatch?.date || '-'}
                    </td>
                    <td
                      className="p-3 border-r font-bold text-xs align-top bg-white bg-opacity-20"
                      title={r.tMatch?.particulars}
                    >
                      {r.tMatch?.particulars || '-'}
                    </td>
                    <td className="p-3 border-r font-mono text-xs align-top bg-white bg-opacity-20">
                      {r.tMatch?.vchNo || '-'}
                    </td>
                    <td
                      className={`p-3 border-r font-mono font-bold text-purple-800 text-right align-top bg-white bg-opacity-20 ${
                        r.sysChq &&
                        r.tMatch &&
                        r.sysChq.amount !== r.tMatch.amount
                          ? 'text-red-600'
                          : ''
                      }`}
                    >
                      {r.tMatch ? formatCurrency(r.tMatch.amount) : '-'}
                    </td>
                    <td className="p-3 text-center align-middle">
                      ...
                      r.reconStatus === 'System Only' ? (
                        <button
                          onClick={() =>
                            setMapModal({ row: r, action: 'CREATE' })
                          }
                          className="text-blue-600 font-bold text-xs hover:underline flex items-center mx-auto"
                        >
                          <Link size={14} className="mr-1" /> Map
                        </button>
                      ) : (
                        <button
                          onClick={() =>
                            setMapModal({ row: r, action: 'BREAK' })
                          }
                          className="text-orange-600 font-bold text-xs hover:underline flex items-center mx-auto"
                        >
                ...
                {filteredRows.length > 150 && (
                  <tr>
                    <td
                      colSpan="14"
                      className="p-4 text-center text-gray-500 font-bold bg-gray-50"
                    >
                      ... {filteredRows.length - 150} more rows. Export to see
                      all.
                    </td>
                  </tr>
                )}
                {filteredRows.length === 0 && (
                  <tr>
                    <td colSpan="14" className="p-4 text-center text-gray-500">
                      No records match this filter/search.
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded shadow-xl w-[600px]">
              ...
              <div className="bg-gray-100 p-4 rounded mb-4 text-sm border">
                ...
                {mapModal.row.sysChq && (
                  <p>
                    <strong>Sys:</strong> {mapModal.row.sysChq.customer} |{' '}
                    {formatCurrency(mapModal.row.sysChq.amount)}
                  </p>
                )}
                {mapModal.row.bMatch && (
                  <p>
                    <strong>Bank:</strong> {mapModal.row.bMatch.desc} |{' '}
                    {formatCurrency(mapModal.row.bMatch.amount)}
                  </p>
                )}
                {mapModal.row.tMatch && (
                  <p>
                    <strong>Tally:</strong> {mapModal.row.tMatch.particulars} |{' '}
                    {formatCurrency(mapModal.row.tMatch.amount)}
                  </p>
                )}
              </div>
              ...
              <form onSubmit={handleSaveMapping}>
                {mapModal.action === 'CREATE' && (
                  <div className="space-y-4 mb-4">
                    {!mapModal.row.sysChq && (
                      <div>
                        ...
                        >
                          ...
                          {cheques
                            .filter(
                              (c) => !c.deleted && !matchedSysIds.has(c.id)
                            )
                            .map((c) => (
                              <option key={c.id} value={c.id}>
                                {c.customer} | {formatCurrency(c.amount)}
                              </option>
                            ))}
                        </select>
                      </div>
                    ...
                    {!mapModal.row.bMatch && (
                      <div>
                        <label className="font-bold text-sm block mb-1 text-blue-800">
                          Link to Bank Statement
                        </label>
                        <select
                          name="bankId"
                          className="w-full border p-2 rounded text-sm bg-blue-50"
                        >
                          <option value="">-- None --</option>
                          {bankData
                    ...
                    {!mapModal.row.tMatch && (
                      <div>
                        <label className="font-bold text-sm block mb-1 text-purple-800">
                          Link to Tally Voucher
                        </label>
                        <select
                          name="tallyId"
                          className="w-full border p-2 rounded text-sm bg-purple-50"
                        >
                          <option value="">-- None --</option>
                          {tallyData
                            .filter((t) => !matchedTallyIds.has(t.id))
                            .map((t) => (
                ...
                {mapModal.action === 'BREAK' && (
                  <p className="text-sm text-orange-600 mb-4 font-bold">
                    This will break the current match and return the items to
                    the Unmapped list.
                  </p>
                )}
                <label className="block font-bold text-sm text-red-600 mb-1">
                ...
                <input
                  ...
                  className="w-full border p-2 rounded border-red-300 mb-4"
                />
  ...
  const ChequeRegister = () => {
    ...
    const toggleAll = () => {
      if (selectedCheques.size === filteredCheques.length)
    ...
    const handleAdminBulkDelete = () => {
      if (selectedCheques.size === 0)
        return alert('Select cheques to delete first.');
      if (
        !window.confirm(
          `Are you sure you want to bulk delete ${selectedCheques.size} cheques?`
        )
      )
        return;
      const updatedCheques = cheques.map((c) =>
        selectedCheques.has(c.id) ? { ...c, deleted: true } : c
      );
      updateCheques(updatedCheques);
      logAction(
        'Bulk Delete',
        'Cheques',
        `Admin bulk deleted ${selectedCheques.size} cheques.`
      );
      setSelectedCheques(new Set());
    };
    ...
    const handleBulkSubmit = () => {
      ...
      validEntries.forEach((entry) => {
        if (
          cheques.some(
            (c) =>
              String(c.chqNo) === String(entry.chqNo) &&
              String(c.bank).toLowerCase() ===
                String(entry.bank).toLowerCase() &&
              !c.deleted
          )
        ) {
      ...
      <div className="p-6">
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center">
            ...
            {isAdmin && selectedCheques.size > 0 && (
              <button
                ...
                className="bg-red-600 text-white px-3 py-1 rounded text-xs font-bold hover:bg-red-700 flex items-center shadow"
              >
          ...
          <div className="flex space-x-3">
            ...
            <button
              onClick={() => exportToExcel(filteredCheques, 'BTC_Cheques')}
              className="bg-green-600 text-white px-3 py-2 rounded text-sm font-bold flex items-center hover:bg-green-700"
            >
                  ...
                  <th className="p-3 w-10 text-center">
                    <input
                      ...
                      checked={
                        ...
                        selectedCheques.size === filteredCheques.length
                      }
                    />
                  </th>
                )}
              ...
              {filteredCheques.map((c) => (
                <tr
                  ...
                  className={`border-b hover:bg-blue-50 ${
                    selectedCheques.has(c.id) ? 'bg-blue-50' : ''
                  }`}
                >
                  {isAdmin && (
                  ...
                  <td className="p-3 font-medium">{c.chqDate}</td>
                  <td className="p-3 text-gray-500 text-xs">{c.enteredAt}</td>
                      ...
                      <tr key={entry.id} className="border-b">
                        <td className="p-1">
                          <input
                            ...
                            onChange={(e) => {
                              const n = [...bulkEntries];
                              n[index].chqDate = e.target.value;
                              setBulkEntries(n);
                            }}
                          />
                        </td>
                        <td className="p-1">
                          <input
                            ...
                            onChange={(e) => {
                              const n = [...bulkEntries];
                              n[index].entryDate = e.target.value;
                              setBulkEntries(n);
                            }}
                          />
                        </td>
                        <td className="p-1">
                          <input
                            ...
                            onChange={(e) => {
                              const n = [...bulkEntries];
                              n[index].customer = e.target.value;
                              setBulkEntries(n);
                            }}
                          />
                        </td>
            ...
            <div className="bg-white p-6 rounded shadow-lg w-[600px]">
              ...
              <form
                onSubmit={(e) => {
                  ...
                  if (!reason)
                    return alert(
                      'A mandatory reason is required to edit this record.'
                    );
                  ...
                  const newAmount = parseFloat(form.amount.value);
                  ...
                  logAction(
                    'Edit',
                    'Cheque',
                    `Edited Chq ${editData.chqNo} (${editData.customer}). Reason: ${reason}`
                  );
                  setEditData(null);
                }}
                className="text-sm"
              >
                <div className="grid grid-cols-2 gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      name="customer"
                      required
                      defaultValue={editData.customer || ''}
                      className="w-full border p-2 rounded"
                    />
                  </div>
                  <div>
                ...
                <div className="bg-orange-50 p-3 rounded mb-4 border border-orange-100">
                  <label className="block font-bold text-orange-800 mb-1">
                  ...
                  <input
                    ...
                    type="text"
                    required
                    placeholder="Why are you editing this cheque?"
                    className="w-full border p-2 rounded border-orange-300"
                  />
                </div>
      ...
      reader.onload = (evt) => {
        try {
          ...
          logAction(
            'Import',
            'Customer Master',
            `Imported ${addedCount} new customers`
          );
      ...
      <div className="p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold flex items-center">
            <Users className="mr-2" /> Master Customers
          </h2>
          <div className="flex space-x-3">
            <button
              onClick={() => exportToExcel(customers, 'BTC_Customers')}
              className="bg-green-600 text-white px-4 py-2 rounded flex items-center text-sm font-bold shadow hover:bg-green-700"
            >
              <Download size={16} className="mr-2" /> Export Excel
            </button>
                      ...
                      <button
                        onClick={() => {
                          if (window.confirm('Delete customer?')) {
                            updateCustomers(
                              customers.filter((cust) => cust.id !== c.id)
                            );
                            logAction(
                              'Delete',
                              'Customer',
  ...
  const UserManagement = () => {
    ...
    const handleUserSubmit = (e) => {
      ...
      if (editUser) {
        if (editUser.id === 1 && !form.active.checked)
          return alert('Master Admin cannot be deactivated.');
        if (editUser.id === 1 && form.role.value !== 'Admin')
          return alert('Master Admin role cannot be changed.');
        ...
        if (usersList.some((u) => u.username === un))
    ...
    return (
      <div className="p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold flex items-center">
            <Shield className="mr-2" /> User Management
          </h2>
          <button
            onClick={() => {
              setEditUser(null);
            ...
            className="bg-blue-600 text-white px-4 py-2 rounded flex items-center text-sm font-bold shadow hover:bg-blue-700"
          >
        ...
        {isUserModalOpen && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded shadow-lg w-[400px]">
              <div className="flex justify-between items-center mb-4 border-b pb-2">
                <h3 className="text-xl font-bold flex items-center">
                  <Key className="mr-2" /> {editUser ? 'Edit User' : 'New User'}
                </h3>
                <X
                  className="cursor-pointer hover:text-red-600"
                  onClick={() => {
              ...
              <form onSubmit={handleUserSubmit} className="space-y-4 text-sm">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      name="role"
                      defaultValue={editUser?.role || 'Team'}
                      className="w-full border p-2 rounded"
                    >
                      <option>Team</option>
                      <option>Admin</option>
                  ...
                  <div className="flex items-center mt-6">
                    <input
                      name="active"
                      type="checkbox"
                      defaultChecked={editUser ? editUser.active : true}
    ...
    <div className="flex h-screen bg-gray-50 text-gray-800">
      <div className="w-64 bg-slate-900 text-white flex flex-col z-10 shrink-0 shadow-xl">
        <div className="p-6 bg-slate-950 text-center border-b border-slate-800">
          <h1 className="font-bold text-lg text-blue-400 leading-tight">
            Barnala Trading Company
          </h1>
        ...
        <nav className="flex-1 overflow-y-auto py-4 space-y-1">
          ...
          >
            <LayoutDashboard size={18} className="inline mr-3" /> Dashboard
          </button>
          <button
            onClick={() => setCurrentScreen('Cheque Register')}
            className={`w-full text-left px-6 py-3 text-sm font-medium ${
              currentScreen === 'Cheque Register'
                ? 'bg-blue-600 border-l-4 border-blue-400'
                : 'hover:bg-slate-800'
            }`}
          >
            <List size={18} className="inline mr-3" /> Cheque Register
          </button>
          <div className="px-6 mt-4 mb-2 text-xs font-bold text-slate-500 uppercase">
            Reconciliation Engine
          </div>
          <button
            onClick={() => setCurrentScreen('Upload Statements')}
            className={`w-full text-left px-6 py-3 text-sm font-medium ${
              currentScreen === 'Upload Statements'
                ? 'bg-blue-600 border-l-4 border-blue-400'
                : 'hover:bg-slate-800'
            }`}
          >
            <UploadCloud size={18} className="inline mr-3" /> Upload Bank/Tally
          </button>
          <button
          ...
          >
            <CheckCircle size={18} className="inline mr-3" /> Auto-Match Status
          </button>
          <div className="px-6 mt-4 mb-2 text-xs font-bold text-slate-500 uppercase">
            System
          </div>
          <button
            onClick={() => setCurrentScreen('Master Customers')}
            className={`w-full text-left px-6 py-3 text-sm font-medium ${
              currentScreen === 'Master Customers'
                ? 'bg-blue-600 border-l-4 border-blue-400'
                : 'hover:bg-slate-800'
            }`}
          >
            <Users size={18} className="inline mr-3" /> Master Customers
          </button>
          {isAdmin && (
            <button
            ...
            >
              <Shield size={18} className="inline mr-3" /> User Management
            </button>
          )}
          <button
          ...
          >
            <FileText size={18} className="inline mr-3" /> Audit Trail
          </button>
        </nav>
        <div className="p-5 bg-slate-950 border-t border-slate-800 flex justify-between items-center">
          <div>
            <p className="text-sm font-bold">{currentUser.name}</p>
            <p className="text-xs text-slate-400">{currentUser.role} Role</p>
          </div>
          <button
        ...
        <main className="flex-1 overflow-y-auto">
          ...
          {currentScreen === 'Audit Trail' && (
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-2xl font-bold flex items-center">
                  <FileText className="mr-2" /> Audit Trail
                </h2>
                <button
                  onClick={() => exportToExcel(auditTrail, 'BTC_Audit_Trail')}
                  className="bg-green-600 text-white px-4 py-2 rounded flex items-center text-sm font-bold shadow hover:bg-green-700"
                >
                  <Download size={16} className="mr-2" /> Export Excel
                </button>
