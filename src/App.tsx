          ...
          <table className="w-full text-[11px] text-left border-collapse">
            <thead className="bg-slate-900 text-white sticky top-0 uppercase tracking-widest z-10 font-black">
              <tr>
                ...
                <th colSpan="2" className="p-2 text-center border-r border-slate-800 bg-blue-900 text-blue-200 tracking-widest font-black italic underline decoration-blue-700">Bank Statement (Zone 2)</th>
            ...
            <tbody>
              {rows.map(r => (
                <tr key={r.id} className={`${r.color} border-b border-slate-50 hover:brightness-95 transition-all`}>
                  ...
                  <td className="p-3 border-r border-slate-100 text-slate-500 font-bold truncate max-w-[200px] italic text-[9px]">{r.bank ? r.bank.desc : "-"}</td>
...
function ChequeRegisterModule({ cheques, save, customers, mappings, logAudit, currentUser }) {
  ...
  const [bulkRows, setBulkRows] = useState([{ id: 1, type: "Payment", eDate: new Date().toISOString().split("T")[0], cDate: "", cust: "", bank: "", no: "", amt: "", status: "Pending" }]);
        ...
        <table className="w-full text-sm text-left border-collapse">
          <thead className="bg-slate-50 border-b text-[10px] font-black uppercase text-slate-400 tracking-widest">
            <tr>
              ...
              <th className="p-6 font-mono">Chq No</th>
...
function UploadModule({ bankData, tallyData, save, logAudit }) {
  const up = (e, t) => {
    ...
    const r = new FileReader(); r.onload = (evt) => {
      ...
      let add = 0, dp = 0;
      if (t === "B") {
        const hIdx = rows.findIndex(r => r.some(c => String(c).toLowerCase().includes("transaction id")));
        if (hIdx === -1) return alert("Invalid file. Header 'Transaction ID' missing.");
        const headers = rows[hIdx].map(h => String(h).trim());
        const raw = rows.slice(hIdx + 1);
        const nl = [...bankData];
        raw.forEach(rowArr => {
          const row = {}; headers.forEach((h, i) => row[h] = rowArr[i]);
        ...
        if (hIdx === -1) return alert("Invalid file. Header 'Particulars' missing.");
        const headers = rows[hIdx].map(h => String(h).trim());
        const raw = rows.slice(hIdx + 1);
        const nl = [...tallyData];
        raw.forEach(rowArr => {
...
function UserManagementModule({ usersList, save, logAudit }) {
  ...
  return (
    <div className="p-10 font-sans">
      <div className="flex justify-between items-center mb-10"><h2 className="text-4xl font-black tracking-tighter italic uppercase underline decoration-indigo-200 decoration-8 underline-offset-[-2px]">Clearance Protocols</h2><button onClick={() => setM({})} className="bg-blue-600 text-white px-10 py-4 rounded-3xl text-xs font-black shadow-xl tracking-[0.2em] uppercase shadow-blue-100">NEW OPERATOR</button></div>
...
function AuditTrailModule({ auditTrail }) {
  ...
  return (
    <div className="p-10 w-full h-full flex flex-col font-sans animate-in fade-in duration-700">
      <h2 className="text-4xl font-black text-slate-800 mb-10 italic tracking-tighter uppercase underline decoration-indigo-200 decoration-8 underline-offset-[-2px]">Security History</h2>
